...
const SurveyManagementTable = (props: Props) => {
                        ...
                        <thead>
                            {headerGroups.map((headerGroup) => (
                                <tr {...headerGroup.getHeaderGroupProps()}>
                                    <th className='min-w-[160px] max-w-[200px]'>Survey Nudge Content</th>
                                    <th>Survey Name</th>
                                    <th>Priority</th>
                                        ...
                                        <td className='text-nowrap cursor-pointer'>
                                            <div className=''>
                                                <span className='text-[14px] p-1'>
                                                    ...
                                                    {moment(row.original.endAt).format('MMMM D, YYYY, h:mm A')}
                                                </span>
                                            </div>
                                        </td>
                                        <td>
                                            <div className='grid grid-flow-col-dense gap-2 w-26'>
                                                <Link href={`questions/${row.original.id}`}>
                                                    <Tooltip title="View Question" arrow placement="bottom">
                                                        <button
                                                ...
                                                {row.original.isEdit === true && (
                                                    <Tooltip title="Edit Survey" arrow placement="bottom">
                                                        <button
                                                            aria-label="Edit Survey"
                                                            className='w-9 h-9 text-center rounded flex justify-center items-center bg-slate-200'
                                                            onClick={() => editSurveyModal(row.original)}
                                                        >
                                                            <FaPencil size={16} />
                                                        </button>
                                                    </Tooltip>
